    ...
    <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex h-16 items-center justify-between px-6">
        <div className="flex items-center space-x-4">
          {/* Remove navigation icons here, keep only POS logo and name */}
        ...
        <div className="flex items-center space-x-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem onClick={() => setIsWaiterOrderCountView(true)}>
                <BarChart3 className="h-4 w-4 mr-2" />
                {language === "hi" ? "वेटर ऑर्डर काउंट" : "Waiter Order Count"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsAnalyticsView(true)}>
                <BarChart3 className="h-4 w-4 mr-2" />
                {language === "hi" ? "सेल्स रिपोर्ट" : "Sales Reports"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsInventoryView(true)}>
                <Package className="h-4 w-4 mr-2" />
                {language === "hi" ? "इन्वेंट्री प्रबंधन" : "Inventory Management"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsStaffView(true)}>
                <User className="h-4 w-4 mr-2" />
                {language === "hi" ? "स्टाफ प्रबंधन" : "Staff Management"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsExpenseView(true)}>
                <DollarSign className="h-4 w-4 mr-2" />
                {language === "hi" ? "खर्च ट्रैकर" : "Expense Tracker"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsRecipeView(true)}>
                <BookOpen className="h-4 w-4 mr-2" />
                {language === "hi" ? "रेसिपी मैनेजर" : "Recipe Manager"}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setIsLoyaltyView(true)}>
                <Users className="h-4 w-4 mr-2" />
                {language === "hi" ? "ग्राहक लॉयल्टी" : "Customer Loyalty"}
              </DropdownMenuItem>
              <DropdownMenuSeparator />

              <DropdownMenuItem onClick={() => setIsReportsView(true)}>
                <BarChart3 className="h-4 w-4 mr-2" />
                {language === "hi" ? "रिपोर्ट" : "Reports"}
              </DropdownMenuItem>

              <DropdownMenuItem onClick={() => setIsStaffProfileView(true)}>
                <User className="h-4 w-4 mr-2" />
                {language === "hi" ? "स्टाफ:" : "Staff:"} {currentStaff?.name || "Admin"}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
