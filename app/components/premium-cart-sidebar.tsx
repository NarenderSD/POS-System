...
export default function PremiumCartSidebar() {
  ...
  // Refactor countdown/redirect logic to avoid setState-in-render error
  useEffect(() => {
    let timer: NodeJS.Timeout | null = null
    if (showBill && billCountdown > 0) {
      timer = setInterval(() => {
        setBillCountdown((prev) => prev - 1)
      }, 1000)
    }
    return () => {
  ...
  // Separate effect to handle redirect when countdown reaches zero
  useEffect(() => {
    if (showBill && billCountdown === 0) {
      setShowBill(false)
      router.push("/")
    }
  }, [showBill, billCountdown, router])

  // Bill overlay/modal
  const billOverlay = showBill && billOrderId ? (
    <div className="order-bill-print fixed inset-0 z-[9999] bg-white/90 flex justify-center w-screen h-screen overflow-y-auto">
  ...
  return (
    <>
      ...
      {typeof window !== 'undefined' && billOverlay ? createPortal(billOverlay, document.body) : null}
    </>
  )
}
